// cms-admin/app/likes-edit/page.js
"use client";
...
export default function LikesEditPage() {
  const [likedProfiles, setLikedProfiles] = useState([]);
  const [originalLikedProfiles, setOriginalLikedProfiles] = useState([]);
  ...
  const areLikedProfilesEqual = (arr1, arr2) => {
  ...
  // --- Data Fetching ---
  useEffect(() => {
    const fetchLikedProfilesData = async () => {
      try {
        const docRef = doc(db, "screens", "likes");
        ...
        setLikedProfiles(fetchedProfiles);
        setOriginalLikedProfiles(fetchedProfiles);

      } catch (e) {
        console.error("Error fetching liked profiles data:", e);
        setError(e.message);
      } finally {
        setLoading(false);
      }
  ...
  const addProfile = () => {
    ...
    setLikedProfiles(prevProfiles => [
      ...prevProfiles,
      { id: newId, name: '', age: '', date: 'today', image: '' }
    ]);
  };

  const removeProfile = async (idToRemove) => {
    const profileToRemove = likedProfiles.find(p => p.id === idToRemove);
    ...
    setLikedProfiles(prevProfiles => prevProfiles.filter(p => p.id !== idToRemove));
  };
  ...
  const handleImageUpload = async (e, profileId) => {
    ...
    const storageRef = ref(storage, `profile_images/likes/${fileName}`); // Changed storage path

    try {
      // Get the current profile to check for existing image
      const currentProfile = likedProfiles.find(p => p.id === profileId);
  ...
  const hasChanges = useMemo(() => {
    if (loading || !originalLikedProfiles) return false;
    return !areLikedProfilesEqual(likedProfiles, originalLikedProfiles);
  }, [likedProfiles, originalLikedProfiles, loading]);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      setOriginalLikedProfiles(dataToSave.profiles);
    } catch (e) {
  ...
  // --- Render Logic ---
  if (loading) return <div className="p-8 text-center text-gray-700 w-full min-h-screen flex items-center justify-center">Loading liked profiles editor...</div>;
  ...
  return (
    <div className="w-full min-h-screen p-8 bg-white">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">Edit Liked Profiles</h1>
      ...
      <div className="mb-6 border p-4 rounded-md bg-gray-50">
        ...
        {likedProfiles.length === 0 && <p className="text-gray-600 mb-2">No liked profiles added yet.</p>}
        {likedProfiles.map((profile, index) => (
          <div key={profile.id} className="border-b border-gray-200 pb-4 mb-4 last:border-b-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label htmlFor={`date-${profile.id}`} className="block text-gray-700 text-sm font-bold mb-1">Date Category:</label>
                <select
                  id={`date-${profile.id}`}
                  className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  value={profile.date}
                  onChange={(e) => handleProfileChange(profile.id, 'date', e.target.value)}
                >
